...
class DeployAgentClient:
    ...
    async def deploy(self, repo_url: str, branch: str, service_name: str) -> str:
        ...
        try:
            ...
            # If using Dockerfile, replace npm ci. If using Buildpacks, delete lockfiles so it uses npm install.
            ...
            if os.path.exists(dockerfile_path):
                ...
                for lockfile in ["package-lock.json", "yarn.lock", "pnpm-lock.yaml"]:
                    lock_path = os.path.join(source_dir, lockfile)
                    if os.path.exists(lock_path):
                        os.remove(lock_path)

            # Use real Firebase credentials so preview environments can test data saving
            firebase_project_id = "MY_GCP_PROJECT"
            firebase_api_key = "REDACTED_FIREBASE_API_KEY"
            firebase_auth_domain = "MY_GCP_PROJECT.firebaseapp.com"
