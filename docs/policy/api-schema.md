...
```
packages/shared/
├─ python/poc_foundry_shared/      # Pydantic v2（API の正本）
│  ├─ agent.py
│  ├─ profile.py
│  ├─ charter.py
│  ├─ issue.py
...
- TS は **生成物のみ**。手書き型は禁止
- フロントは `import { AgentSchema, type Agent } from "@poc-foundry/shared"` で読む
...
- フロント側は `@poc-foundry/shared` の zod スキーマで **受信時に必ず parse**（失敗 = テスト失敗）
