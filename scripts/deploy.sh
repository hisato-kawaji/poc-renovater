...
gcloud run deploy poc-foundry-api \
...
API_URL=$(gcloud run services describe poc-foundry-api --region ${REGION} --project ${PROJECT_ID} --format="value(status.url)")
...
gcloud run deploy poc-foundry-web \
